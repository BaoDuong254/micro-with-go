...
### Windows one-click fix for Docker Desktop image pull errors
...
- detect current `ride-sharing/*` images used by workloads in the `default` namespace
- import matching local Docker images into the Docker Desktop Kubernetes node containerd
- rollout restart affected deployments and wait until they are healthy

Optional flags:

```powershell
# Different namespace
.\infra\development\docker\sync-tilt-images.bat -Namespace my-namespace

# Sync only, without rollout restart
.\infra\development\docker\sync-tilt-images.bat -SkipRestart

# Preview actions without changing cluster state
.\infra\development\docker\sync-tilt-images.bat -DryRun
```
